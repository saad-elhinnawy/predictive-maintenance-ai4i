...
exports.sendMilestoneUpdate = sendMilestoneUpdate;
...
function buildMilestoneHtml(opts) {
...
function buildMilestoneText(opts) {
    const { name, carModel, orderId, milestone, nextMilestone } = opts;
    const trackingUrl = `${process.env.APP_URL ?? 'https://yourdomain.com'}/#/tracking/${orderId}`;
...
// ─── Public API ───────────────────────────────────────────────────────────────
async function sendMilestoneUpdate(opts) {
...
async function sendOrderConfirmation(email, name, carModel, orderId) {
